...
namespace vraijeux
{
    class Joueur
    {
      ...
      public string getNom()
        ...
        {
           if(this.niveauVie + (this.vieMax / 10) >= this.vieMax)
            {
                this.niveauVie = vieMax;
            }
            else
            {
                this.niveauVie = this.niveauVie + (this.vieMax / 10);
            }
        }
        ...
        /*public void rencontreAleatoire(monstre monstre)
        {
            while(this.getVie()>0 && monstre.getVie()> 0)
            {
                Console.WriteLine("voici les stats du monstre :\n "+monstre.ToString());
                this.attaquerMonstre(monstre);
                Console.WriteLine("il lui reste : " + monstre.getVie()+ "de vie");
