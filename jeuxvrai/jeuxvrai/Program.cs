...
namespace jeuxvrai
{
    class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            Joueur joueur1 = new Joueur("kylian", 50, 2000, 50, debut);
            ...
            Console.WriteLine("\n Vous avez le choix entre plusieurs zones , veuillez choisir votre zone parmis les zones disponible :\n");
            jeux.afficherZone();
            string choixZone = Console.ReadLine();
           

            while(joueur1.getVie() > 0 && jeux.listZone() != 0)
            {
                if(choixZone == "plaine")
                {
                    jeux.changerZone(plaine);
                    while (plaine.nbmonstre() != 0)
                    {
                        jeux.rencontreAleatoire();
                        if (joueur1.getVie() > 0 && plaine.nbmonstre() != 0)
                        {
                    ...
                    if(joueur1.getVie() > 0)
                    {
                        jeux.retirerZone(plaine);
                        if (jeux.listZone() == 0)
                        {
                            Console.WriteLine("\n Bravo vous avez terminer le jeux ! FELICITATION \n FELICITATION !");
                        }
                        else
                        {
                            Console.WriteLine("\n Vous avez finis votre zone ,  veuillez choisir votre zone parmis les zones disponible :\n");

                            jeux.afficherZone();
                            choixZone = Console.ReadLine();
                        }
                       
                    }
                        
                }
                if(choixZone == "montagne")
                {
                    jeux.changerZone(montagne);
                    while (montagne.nbmonstre() != 0)
                    {
                        jeux.rencontreAleatoire();
                        if (joueur1.getVie() > 0)
                        {
